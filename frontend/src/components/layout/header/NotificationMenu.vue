<template>
  <div class="relative" ref="dropdownRef">
    ...
    >
      ...
      <ul class="flex flex-col h-auto overflow-y-auto min-h-[350px] custom-scrollbar">
        <li v-for="notification in system_store.notifications.logs" :key="notification.id" @click="handleItemClick($event,notification.id,notification.module)">
          <a
          ...
          >
            <span class="relative block w-full h-10 rounded-full z-1 max-w-10">
              <!-- <img :src="notification.userImage" alt="User" class="overflow-hidden rounded-full" /> -->
               <Icon class="text-4xl" :icon="notification.icon" />
              <span
...
const handleItemClick = (event,id,module) => {
  event.preventDefault()
  // Handle the item click action here
  router.push(`/view/${module}/${id}`);
